...
class AttSacMeta():
    ...
    def __init__(self, env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
        ...
        obs_dim = env.att_observation_space.shape[0]
        act_dim = env.att_action_space.shape[0]

        otheract_dim = env.def_action_space.shape[0]

        act_limit = env.att_action_space.high[0]
        ...
        ac_kwargs['action_space'] = env.att_action_space
        ...
        with tf.variable_scope('att', reuse=True):
            ...
            _, self.pi_next, self.logp_pi_next, _, _ = actor_critic(self.x2_ph, self.a_ph, **ac_kwargs)
            
            # ty: logp_phi_next, make sure the action is from the current policy
            # self.logp_phi_next = core.gaussian_likelihood(self.pi_next, self.meta_mu_next, self.meta_log_std_next)
    ...
    def act(self, o, t, deterministic=False):
        if t > self.start_steps:
            ...
            # sketchy code
            return self.test_env.att_action_space.sample()
