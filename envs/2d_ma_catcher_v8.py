...
class CatcherEnv(gym.Env):
	...
	def _initDefObsSpace(self):
		...
		lb = np.zeros(2 + 2, dtype=np.float32)
		ub = np.ones(2 + 2, dtype=np.float32) * self.obs_limit
	...
	def _initTarget(self):
		...
		return np.ravel(self.tarPossNdarray)
	...
	def getDefObs(self, obs_all):
		...
		# ty: now only return defender's current pos and the current target
		return np.append(obs_all[:2], np.array(self.tarPoss[self.stage]))
	...
	def _calcReward_ultimate2(self):
		...
		# no distance reward shaping
		# only reward if reach target
	...
	def _calcReward_ultimate3(self):
		...
		# reward if reach target + distance reward shaping + penalty if further
		...
		for i in range(self.num_target):
			...
			if (tarAttDist <= 15.0) and (attDefDist <= 20.0):
				# attacker caught when attacking
				return (tarR + shapingFlag * rewardplus + shapingFlag2*rewardplus2-shapingFlag3*rewardplus3, -tarR), True, {"done": "attacker caught"}
			elif (tarAttDist <= 15.0) and (self.penetrationCtrs[i] >= self.penetrationTime):
				# attacker completes attack
				return (shapingFlag * rewardplus + shapingFlag2*rewardplus2-shapingFlag3*rewardplus3, tarR), True, {"done": "target attacked"} #ty: focus on reaching target
			elif (tarAttDist <= 15.0) and (self.penetrationCtrs[i] < self.penetrationTime):
				# attacker ongoing attack
				self.penetrationCtrs[i] += 1
				return (-0.5 + shapingFlag * rewardplus + shapingFlag2*rewardplus2-shapingFlag3*rewardplus3, 1.0), False, {"done": None}
			else:
				# other cases
				self.penetrationCtrs[i] = 0
		
		return (-0.5 + shapingFlag * rewardplus + shapingFlag2*rewardplus2 -shapingFlag3*rewardplus3, -0.5), False, {"done": None}
	...
	def step(self, action):
		...
		if self.num_steps >= MAX_STEPS:
			...
			rs, done, info = self._calcReward_ultimate3()
